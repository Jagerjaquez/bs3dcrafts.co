...
import { ArrowRight, Layers, Zap, Shield, Sparkles, Camera, Upload, Star, TrendingUp, Quote } from 'lucide-react'
import { HomepageLoadingSkeleton } from '@/components/homepage-loading-skeleton'
...
export function HomepageContent() {
  ...
  const { data: homepageData, isLoading, isError, error } = useHomepageContent()
  ...
  // Handle error state gracefully - data will always be available due to fallback
  if (isError) {
    console.error('Homepage content error:', error)
    // Continue with fallback data - the hook provides fallback data automatically
  }

  // Use the data from SWR (includes fallback data if API fails)
  const content = homepageData!
  const hero = content.hero!
  const testimonials = content.testimonials!
  const stats = content.stats || { customers: 1000, products: 5000, satisfaction: 99, support: '24/7' }

  // Check if newsletter is enabled in site settings (with fallback)
  const newsletterEnabled = settings?.features?.newsletter !== false // Default to true
              ...
              <Link href={hero.buttonUrl || '/products'}>
                <Button size="lg" className="text-xl px-12 py-8 bg-gradient-to-r from-primary via-secondary to-accent hover:opacity-90 transition-all hover-glow shadow-2xl shadow-primary/60 group relative overflow-hidden animate-pulse-glow">
                  ...
                  <span className="relative flex items-center gap-3">
                    ...
                    <span className="font-bold tracking-wide">{hero.buttonText}</span>
                ...
                <div className="glass rounded-2xl overflow-hidden hover-glow transition-all duration-300">
                  <div className="relative h-64 overflow-hidden">
                    <LazyImage 
                      ...
                      priority={i <= 2} // Prioritize first 2 images above the fold
                      placeholder="blur"
                      blurDataURL="data:image/jpeg;base64,"
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {testimonials.map((testimonial, i) => (
              <div key={testimonial.id} className="animate-slide-in-bottom" style={{ animationDelay: `${i * 0.1}s` }}>
      ...
      <section className="py-24 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
              { number: `${stats.customers}+`, label: 'Mutlu Müşteri', icon: '😊' },
              { number: `${stats.products}+`, label: 'Teslim Edilen Ürün', icon: '📦' },
              { number: `${stats.satisfaction}%`, label: 'Müşteri Memnuniyeti', icon: '⭐' },
              { number: stats.support, label: 'Destek Hizmeti', icon: '💬' }
            ].map((stat, i) => (
          ...
          <div className="container mx-auto px-4 relative z-10">
            <div className="max-w-4xl mx-auto">
              <div className="glass rounded-3xl p-12 neon-border text-center animate-zoom-in">
                ...
                <h2 className="text-4xl md:text-5xl font-bold mb-4 text-white">
                  {content.newsletter?.title || 'Kampanyalardan'} <span className="gradient-text">Haberdar Olun</span>
                </h2>
                
                <p className="text-xl text-gray-300 mb-8 max-w-2xl mx-auto">
                  {content.newsletter?.subtitle || 'Yeni ürünler, özel indirimler ve kampanyalardan ilk siz haberdar olun'}
                </p>
